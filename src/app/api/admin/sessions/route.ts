...
export async function GET(req: NextRequest) {
  try {
    ...
    const countRes = await pool.query(`SELECT COUNT(*) as total FROM quiz_sessions qs ${where.replace('qs.','')}`, params);
    const { rows } = await pool.query(`SELECT qs.*, u.full_name, u.phone,
      ...
      FROM quiz_sessions qs JOIN users u ON qs.user_id=u.id ${where} ORDER BY qs.started_at DESC LIMIT $${idx++} OFFSET $${idx++}`, [...params, limit, offset]);
